...
`packages/sdk/src/reservine-button.component.svelte` is the only booking button/modal/drawer implementation. React, Vue, Svelte, and Angular code under `packages/sdk/src/adapters` must remain thin lifecycle/property/event bridges; never reproduce booking behavior inside an adapter.
