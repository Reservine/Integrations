...
function setStyle(element: HTMLElement, style: string, value: string) {
  const property = style.replace(/[A-Z]/g, (letter) => `-${letter.toLowerCase()}`);
  const current = element.style.getPropertyValue(property);
  element.style.setProperty(property, value);

  return () => {
    if (current) element.style.setProperty(property, current);
    else element.style.removeProperty(property);
  };
}

function scrollIntoView(initialTarget: Element) {
  const { documentElement, body, scrollingElement } = document;

  const root = scrollingElement || documentElement;
  let target: Element | null = initialTarget;
